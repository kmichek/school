...
export default class PlusMinus extends Component {
  ...
  constructor() {
    ...
    if (! this.teacher.has('__multiply3')){
      this.teacher.on('__multiply3', function(maxResult, range1, range2, count) {
  ...
  @action start(maxResult, range1, range2, count) {
    ...
    while (counter > 0){
      ...
      let random2 = Math.floor(Math.random() * (high1 - high2) ) + high2;
