...
export default class Multiply5 extends Component {
  ...
  constructor() {
    ...
    if (!this.teacher.has('__multiply9')) {
      this.teacher.on('__multiply9', function (maxResult, range1, range2, count) {
  ...
  @action start(maxResult, range1, range2, count) {
    ...
    while (counter > 0 && max < 100) {
      ...
      if ((random1 > 0) && ((random1 * 9) <= high1)) {
        let question = random1 + ' * ' + 9;
