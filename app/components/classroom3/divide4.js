...
export default class PlusMinus extends Component {
  ...
  constructor() {
    super(...arguments);
    let self = this;
    if (!this.teacher.has('__divide4')) {
      this.teacher.on('__divide4', function (maxResult, range1, range2, count) {
  ...
  @action start(maxResult, range1, range2, count) {
    ...
    while (counter > 0 && max < 100) {
      max++;
      let random1 = (Math.floor(Math.random() * (high1 - high2)) + high2) * 4;
      if (random1 > 0 && random1 <= high1) {
        let question = random1 + ' / ' + 4;
